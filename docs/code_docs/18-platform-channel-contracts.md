...
| `pushRules` | `{json: String?, nextBoundaryMs: Long}` | `pushWebBlocklist`'s twin: absent / non-JSON-array `json` = **no-op** (clearing needs an explicit `"[]"`; Dart sends it absent when the entries equal its last pushed snapshot, so a resume / boundary push marshals the boundary alone), unchanged `json` skips the prefs rewrite; `nextBoundaryMs` (read as `Number`, clamped to ≥ 0) is **always** written; then `service.refreshRules()` re-reads both into `RuleEngine` + the boundary mirror, re-parsing only on a changed snapshot string | `true` | `Future<bool> pushRules(String? json, int nextBoundaryMs)` — `false` when the call failed or there is no native side; `syncRules` then reports nothing so Dart never records a snapshot native did not take |
